...
func _ready():
	...
	get_node("texto/acertos").set_text(str(game.acertos)) 
	get_node("texto/erros").set_text(str(game.erros))
	get_node("texto/moedas").set_text(str(game.poupanca))
	#APRESENTA OS PRÊMIOS QUE JOGADOR COMPROU
	for i in range(game.premios.size()):
		if game.premios[i][2] == true:
			get_node("texto/premio"+str(j)).set_texture(load("res://sprites/"+str(game.premios[i][0])+".png"))
			j += 1
